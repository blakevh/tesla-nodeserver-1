  ...
  class TeslaInterface {
    ...
    async getAccessToken(forceRefresh = false) {
      ...
      if ((tokenExpiry && new Date().valueOf() + 60000 > tokenExpiry.valueOf())
        ...
        try {
          const refreshToken = await this.refreshTokens(oauth);

          if (refreshToken && refreshToken.response) {
            oauth = refreshToken.response.body;

            const statusCode = refreshToken.response.statusCode;
            if (statusCode != 200) {
              // this has been updated for TeslaJS error handling for refreshToken.
              // TODO: may need to revisit what to return or throw here.
          ...
          logger.debug("tesla_v3.getAccesToken %o", err);
          if (err.code === 401) {
